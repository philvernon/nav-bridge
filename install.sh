...
install_tmux_config() {
    ...
    if [[ ! -f "$package_config" ]]; then
        printf 'error: platform tmux config is missing: %s\n' "$package_config" >&2
        exit 1
    fi
    ...
    append_source "$config_file" "set -g @nav_bridge_dir \"$project_dir\""
    append_source "$config_file" "source-file \"$package_config\""
}
...
install_sway_config() {
    ...
    append_source "$config_file" "set \$nav_bridge_dir \"$project_dir\""
    append_source "$config_file" "include \"$project_dir/linux/sway.conf\""
}
...
install_launch_agent() {
    ...
    service="gui/$(id -u)/com.phil.nav-bridge"
    ...
    sed \
        ...
        -e "s|__NAV_LISTENER__|$(escape_sed_replacement "$project_dir/macos/nav-listener.sh")|g" \
...
Darwin)
    ...
    ;;
Linux)
    install_tmux_config "$project_dir/linux/tmux.conf"
    install_sway_config
    ;;
esac
